...
#' Synthetic data set based on 120 m radius road and pad searches of all 100
#'   turbines at a theoretical site. Bat data only
#'
#' Complete data set for fatality estimation at a wind power facility.
#'   Included are SE, CP, SS, DWP, and CO data.
#'
...
#' A complete example data set for estimating fatalities from 60 m cleared 
#'   plots at 23 out of 100 searches at a wind power facility
#'
#' Data on carcass observations (CO) from road and pad searches out to 120 m
#'   from each of 100 turbines at a theoretical site, field trials for 
#'   estimatingcarcass persistence (CP) and searcher efficiency (SE), search 
#'   schedule (SS) parameters (for example, which turbines were searched on 
#'   which days), and density weighted proportion (DWP) of area searched at 
#'   each turbine (which is an area adjustment factor to account for
...
#'      with \code{"RP"} for carcasses placed on a road or turbine pad, 
#'      \code{"M"} for moderate visibility (e.g., plowed field; short, sparse
#'       vegetation), or \code{"D"} for difficult visibility}
#'   \item{\code{"s1",...,"s5"}}{fate of carcass on the 1st, 2nd, 3rd, 4th,
#'      and 5th search after placement. A value of 1 implies that a carcass
...
#'      turbine was not searched because of snow, or "NS" to indicate the 
#'      search was not scheduled in that location at that time, or "SC" to 
#'      indicate the carcass had been removed by scavengers prior to the
#'      search.}
#' }
...
#'   \item{\code{Visibility}}{indicator for visibility class of the ground,
#'    with \code{"RP"} for carcasses placed on a road or turbine pad, 
#'    \code{"M"} for moderate visibility 
#'    (e.g., plowed field; short, sparse vegetation), or \code{"D"} for 
...
#'    carcass removal is known (e.g., recorded by camera), then \code{Left}
#'    and \code{Right} should be set equal to each other. If a carcass 
#'    persists beyond the last day of the field trial, \code{Left} is the last
#'    time it was observed and \code{Right} is entered as \code{Inf} or 
#'    \code{NA}.}
#' }
#'
...
#'    turbine. In addition, there is a column to indicate the \code{Season}.
#'    A column with search dates and columns for each turbine searched are
#'  required. Other columns are optional.
#' \describe{
#'   \item{\code{SearchDate}}{columns of dates on which at least one turbine 
#'    was searched. Format in this data is \code{"\%Y-\%m-\%d CDT"}, but time 
#'    zone (\code{CDT}) is optional. A time stamp may be included if desired
...
#'   of the searched area for each size (or the fraction of carcasses that 
#'   fall in the searched area).
#' \describe{
...
#'     \code{$SS$SearchDate}. Every date entered here is (and must be) 
#'     included in the search schedule (\code{$SS$SearchDate}}
