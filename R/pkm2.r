...
#' @export
#'
pkm2 <- function(formula_p, formula_k = NULL, data, obsCol = NULL, 
  ...
  cellMM <- cbind(cellMM_p, cellMM_k)
    ...
    ##block diag form
  dataMMbdiag <- as.matrix(Matrix::bdiag(cellMM_p, cellMM_k))
  cellMMbdiag <- as.matrix(Matrix::bdiag(cellMM_p, cellMM_k))
  ...
  MLE <- tryCatch(
           optim(par = betaInit, fn = pkLogLik2, method = "BFGS", hessian = T, 
             cellByCarc = cellByCarc, misses = misses, maxmisses = maxmisses,
             foundOn = foundOn, cellMM = cellMM, nbeta_p = nbeta_p, 
             kFixed = kFixed, betaHi = NULL, betaLo = NULL, 
             reducedCellMM = cellMMbdiag
           ), 
           error = function(x) {NA}
         )   
   MLE$fixedBetas = 0
...
##fix parameters whose values are approaching 0 or 1 and refit
    prelimPKHats = as.vector(cellMMbdiag %*% MLE$par)
    reducedCellMMbdiag = cellMMbdiag
    
    if(any(prelimPKHats > logit(0.99) | prelimPKHats < logit(0.01), na.rm = T)){

        reducedBetaInit = betaInit
        pkHi = NULL
        pkLo = NULL

        while(any(!
            which(MLE$par >= logit(0.99) | MLE$par <= logit(0.01)) %in%
            MLE$fixedBetas)){
        # while(any(prelimPKHats > logit(0.99) | prelimPKHats < logit(0.01), 
            # na.rm = T)){

            pkHi = c(pkHi, which(prelimPKHats >= logit(0.99)))
            pkLo = c(pkLo, which(prelimPKHats <= logit(0.01)))
            
            reducedCellMMbdiag[c(pkHi), ] = 0
            betaHi = which(colSums(reducedCellMMbdiag) == 0)
            reducedCellMMbdiag[pkLo, ] = 0
            betaLo = which(colSums(reducedCellMMbdiag) == 0)
            betaLo = betaLo[!betaLo %in% betaHi]

            whichK = (1:ncol(reducedCellMMbdiag))[-(1:nbeta_p)]

            # if(all(c(betaHi, betaLo) %in% whichK)){
            # reducedCellMMbdiag =

            # }

            reducedBetaInit = 
                betaInit[-c(betaHi, betaLo)] 


        MLE <- tryCatch(
               optim(par = reducedBetaInit, fn = pkLogLik2, 
                 method = "BFGS", hessian = T, 
                 cellByCarc = cellByCarc, misses = misses, 
                 maxmisses = maxmisses,
                 foundOn = foundOn, cellMM = cellMM, 
                 nbeta_p = nbeta_p, 
                 kFixed = kFixed, betaHi = betaHi, betaLo = betaLo, 
                 reducedCellMMbdiag = reducedCellMMbdiag
               ), 
               error = function(x) {NA}
             )
        ...
        betaFixed[colSums(reducedCellMMbdiag) !=0] = MLE$par
        betaFixed[betaHi] = logit(0.99)
        betaFixed[betaLo] = logit(0.01)
        MLE$par = betaFixed
        MLE$fixedBetas = c(betaHi, betaLo)
        prelimPKHats = as.vector(cellMMbdiag %*% MLE$par)
        prelimPKHats[c(pkHi, pkLo)] = NA

        }##while fixing betas loop
    }##if there's a beta problem

  reducedCellMM_p = reducedCellMMbdiag[1:ncell, 1:nbeta_p]
  reducedCellMM_k = reducedCellMMbdiag[-(1:ncell), -(1:nbeta_p)]
  convergence <- MLE$convergence
  betahat <- MLE$par
  betaHessian <- MLE$hessian
  fixedParameters = MLE$fixed
  llik <- MLE$value
  ...
  if(any(fixedParameters > 0)){
    vb = diag(0, ncol(varbeta) + length(fixedParameters))
    vb[-fixedParameters, -fixedParameters] = varbeta
    varbeta = vb
    }

  varbeta_p <- varbeta[1:nbeta_p, 1:nbeta_p]
  cellMean_p <- reducedCellMM_p %*% betahat_p
  cellMean_p[1:length(cellMean_p) %in% pkHi] = logit(1)
  cellMean_p[1:length(cellMean_p) %in% pkLo] = logit(0)
  cellVar_p <- reducedCellMM_p %*% varbeta_p %*% t(reducedCellMM_p)
  cellSD_p <- sqrt(diag(cellVar_p))

  if (length(kFixed) == 0){
    which_k <- (nbeta_p + 1):(nbeta)
    varbeta_k <- varbeta[which_k, which_k]
    cellMean_k <- reducedCellMM_k %*% betahat_k
    cellMean_k[1:length(cellMean_k) %in% (pkHi - ncell)] = logit(1)
    cellMean_k[1:length(cellMean_k) %in% (pkLo - ncell)] = logit(0) 
    cellVar_k <- reducedCellMM_k %*% varbeta_k %*% t(reducedCellMM_k)
  ...
  fixed_p = cellNames[c(pkHi, pkLo)]

  fixedParameters <- 
    c(paste(cellNames, '_p'), paste(cellNames, '_k'))[c(pkHi, pkLo)]
  ...
  output$reducedCellMM_p <- reducedCellMM_p
  output$reducedCellMM_k <- reducedCellMM_k
  ...
  output$fixedParameters <- fixedParameters
