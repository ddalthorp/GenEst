---
title: "Gen Est - A Solar Example with Carcass Size"
author: "D Dalthorp, J Mintz, J Simonis"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Gen Est - A Solar Example with Carcass Size}
...
# Gen Est - A Solar Example with Carcass Size
## Introduction
...
constant over throughout the facility, and finally how different size classes of
birds are affected.  
...
Size Bat: 
```{r pk Size Bat}
pkmSetAICcTab(SE_size_model_set$bat)
SE_models$bat <- SE_size_model_set$bat[[2]]
```
...
Left and Right represent the left (start) and right (end) endpoints of the 
interval over which a carcass went missing.  For further information about CP 
trials and how to format results for use with GenEst, see User Manual Appendix 
A2 and B2.
...
Size Bat: 
```{r cp Size Bat}
cpmSetAICcTab(CP_size_model_set$bat)
CP_models$bat <- CP_size_model_set$bat[[4]]
```
...
SS_data[1:3 , 1:10]
```
(Note that there are 100 turbine columns altogether, t1, ..., t100)

Density weighted proportion:
```{r DWP data}
DWP_data <- solar_PV$DWP
head(DWP_data, 3)
names(DWP_data) <- c("Unit", "bat", "small", "med", "lrg")
```
...
the function SS.  This allows us to specify the exact time intervals over which 
...
dim(summary(M_unit))  # only 163 turbines had observations.
