...
#' @param SSdat List that summarizes the search schedule data and must include
#'  (at a minimum) \code{$days}, \code{$searches}, and \code{$unit}. In addition,
#'  if \code{split_SS} is non-NULL, \code{SSdat} must also include an element
...
#' is returned from the S3 function \code{summary(splits, CI_level = 0.95, ...)},
...
#' 1 - alpha/2 quantiles, where alpha = 1 - CI_level. A graph summarizing the
#' results can be drawn using \code{plot(splits, CI_level, ...)}, which gives
...
calcSplits <- function(M, Aj = NULL,
  ...
  if (nvar == 0){ # no splits...just calculate total M
    ...
    if (split_h$type == "CO"){
      ...
      rate <- calcRate(M, Aj, SSdat$days, searches)
...
#' and each simulation draw). summary(splits, CI_level = 0.95, ...) gives
...
#' @param CI_level desired confidence level for summary CIs (numeric scalar in (0, 1))
...
#'  alpha = 1 - CI_level. A graphical representation of the results can be
#'  produced using \code{plot(splits, CI_level, ...)}.
#' @export
...
#' intervals (for the \code{CI_level} attribute for the splits object). For
...
#'  include either a \code{split_SS} or \code{split_time} variable, the
#'  \code{rate} arg is ignored.
#' @param CI_level desired confidence level to show in box plots
#' @export
plot.splitFull <- function(splits, rate = FALSE, CI_level = 0.95, ...){
