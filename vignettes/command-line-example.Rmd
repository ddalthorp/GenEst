...
The `estghatGenericSize` function produces `n` random draws of generic (i.e., 
...
models across the size classes. `estghatGeneric` is a single-size-class
version of function and `estghatGenericSize` actually loops over 
`estghatGeneric`. The generic $\hat{g}$ is estimated according to a particular 
search schedule. When we pass `averageSS` a full `data_SS` table like we have
here, it will assume the units are named as "unit" with a suffix indicator
and will create the average search schedule across the units.

```{r}
data_SS <- mock$SS
avgSS <- averageSS(data_SS)
ghatsGeneric <- estghatGenericSize(n = 1000, avgSS, pkmModSetSize, 
...
carcasses based on the dates when they are observed, size class values, 
associated covariates, the searcher efficiency and carcass persistence models,
and the search schedule. The carcass-specific detection probabilities (as
opposed to the generic/cell-specific detection probabilities above) are 
therefore calculated before estimating the total number of fatalities. 
Although it is possible to estimate these detection probabilities separately,
they are best interpreted in the context of a full fatality estimation. 
...
eM <- estM(nsim = 1000, data_CO, data_SS, data_DWP, frac = 1,  
        ...
        seed_CP = NULL, seed_ghat = NULL, seed_M = NULL, kFill = NULL,  
        unitCol = "Unit", dateFoundCol = "DateFound", 
        dateSearchedCol = "DateSearched", DWPCol = DWPcolnames,
        sizeclassCol = "Size")
```

### Splitting Fatality Estimations
