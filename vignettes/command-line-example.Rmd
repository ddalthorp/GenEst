...
(`ghat`) and arrival intervals (`As`) for each carcass.
...
fatalities (Mhat) associated with each estimate of detection probability for 
each carcass and weight the estimate by the Density Weighted Proportion (DWP) 
of the relevant area (e.g., around the turbine) where the carcass was found. 
If DWP is not provided, the `rMhat` make a simple assumption that DWP is 1 for
all carcasses (i.e., that all of the possibe area where the carcasses could 
have landed was searched). 
