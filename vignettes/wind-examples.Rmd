...
M_season <- calcSplits(M = Mhat$M, Aj = Mhat$Aj,
  split_SS = "Season", data_SS = data_SS, split_CO = NULL,  data_CO = data_CO)
summary(M_season)
plot(M_season)
```
...
M_month <- calcSplits(M = Mhat$M, Aj = Mhat$Aj,
  split_time = seq(0, max(SSdat$days), by = 30), 
...
M_speciesgroup_and_turbinetype <- calcSplits( M = Mhat$M, Aj = Mhat$Aj,
  split_CO = c("SpeciesGroup", "TurbineType"), data_CO = data_CO)
plot(M_speciesgroup_and_turbinetype)
```
