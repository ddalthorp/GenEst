...
carcasses based on the dates when they are observed, associated covarites, the
...
The `rghatGeneric` function produces `n` random draws of generic (i.e., not
carcass-sepecific) detection probabilities (`ghat`) for each of the 
possible carcass cell combinations across the SE and CP models. The generic
`ghat` is estimated according to a particular search schedule. When we pass 
`rghatGeneric` a full `data_SS` table like we have here, it will assume the
units are named as "unit" with a suffix indicator and will use the average
search schedule across the units.

```{r}
ghatsGeneric <- rghatGeneric(n = 1000, data_SS, model_SE, model_CP, 
