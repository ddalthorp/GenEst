...
generic/cell-specific detection probabilities above) are therefore calculated 
before estimating the total number of fatalities. Although it is possible
to estimate these detection probabilities on their own, they are best 
interpreted together and in the context of DWP and the fraction of the 
facility surveyed. That is, in the context of a full fatality estimation. 

### Carcass-Specific Detection Probability

If a user is interested in estimating carcass-specific probabilities, the
`estghat` function produces `nsim` random draws of carcass-specific detection
probabilities ($\hat{g}$) and arrival intervals ($A_j$):.

```{r, eval=FALSE}
data_CO <- mock$CO
ghatsAjs <- estghat(nsim = 1000, data_CO, data_SS, model_SE, model_CP,
