...
#' @export
#'
estM <- function (data_CO, data_SS, data_DWP, frac = 1, COdate = "DateFound",
                  ...
                  seed_M = NULL, nsim = 1000, max_intervals = 8)
{
  ...
  if (!is.null(sizeCol)) {
    if (!(sizeCol %in% colnames(data_CO))) {
      ...
      sizeclasses <- sort(as.character(unique(data_CO[,
                                                      sizeCol])))
  ...
  est <- estg(data_CO = data_CO, COdate = COdate, data_SS = data_SS,
              SSdate = SSdate, model_SE = model_SE, model_CP = model_CP,
              unitCol = unitCol, sizeCol = sizeCol, nsim = nsim, max_intervals = max_intervals,
              seed_SE = seed_SE, seed_CP = seed_CP, seed_g = seed_g, DWP = DWP)
  gDf <- est$ghatAC * frac
  set.seed(seed_M)
  c_out <- which(rowSums(gDf) == 0)
  if (length(c_out) == 0) {
    n <- length(gDf)
    Mhat <- ((cbinom::rcbinom(n, 1/gDf, gDf)) - (Ecbinom(gDf) -
                                                   1))/gDf
  }
  else {
    ...
    gDf <- gDf[-c_out, ]
    n <- length(gDf)
    Mhat[-c_out, ] <- ((cbinom::rcbinom(n, 1/gDf, gDf)) -
                         (Ecbinom(gDf) - 1))/gDf
  }
                         ...
                         sizeCol = NULL, DWPCol = NULL)
{
  ...
  repName <- "rep"  # hard coded for now, could pass as argument
  if (repName %in% colnames(original_DWP)) {
    long_DWP <- original_DWP
    data_DWP <- subset(long_DWP, subset = rep == 1)[, -which(colnames(long_DWP) == repName)]
    ...
    if (is.null(unitCol)) {
      unitCol <- colnames(data_DWP)[colnames(data_DWP) %in%
                                      colnames(data_CO)]
      if (length(unitCol) == 0) {
        stop("data_DWP and data_CO must have identically-named unit columns")
      }
      if (length(unitCol) > 1) {
        stop(paste("more than one possibility for unitCol (",
                   unitCol, ")", collapse = " "))
      }
    }
    else {
      if (length(unitCol) > 1)
        stop("length(unitCol) must be 1 if provided")
    ...
    if (!is.null(sizeCol)) {
      ...
      if (!is.null(DWPCol)) {
        ...
        if (!is.numeric(data_DWP[, DWPCol]) || anyNA(data_DWP[,
                                                              DWPCol])) {
          stop("data_DWP[, DWPCol] must be numeric with no NA's")
        }
        if (sum(data_DWP[, DWPCol] <= 0 | data_DWP[, DWPCol] >
                1) > 0) {
        ...
        for (coli in possibleNames) {
          if (is.numeric(data_DWP[, coli]) && !anyNA(data_DWP[,
                                                              coli]) && all(data_DWP[, coli] <= 1) && all(data_DWP[,
                                                                                                                   coli] > 0)) {
            if (is.null(DWPCol)) {
              DWPCol <- coli
            }
            else {
              stop("data_DWP must have columns corresponding to data_CO sizes\n",
  ...
  }  # end if for area correction variance
